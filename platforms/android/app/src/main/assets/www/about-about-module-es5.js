    ...
    let AboutPage = class AboutPage {
      constructor(navCtrl, loadingCtrl, platform, http, route, router, toastController) {
        ...
        this.http.get('https://riwayatedilli.com/site/wp-json/wp/v2/pages/1991').subscribe(data => {
          this.details = Array.of(data); // console.log(this.details[0].title.rendered);
        }), error => {
          this.presentToast();
        };
      }
      ...
      removeHTMLInfo(value) {
        if (value) return value.replace(/<\/?[^>]+>/gi, "");
      }
      ...
      ngOnInit() {
        this.postid = this.route.snapshot.paramMap.get('id'); // this.backbttsub();
      }
